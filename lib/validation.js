...
function normalizeSettings(input, currentSettings = null) {
    ...
    const servers = serverInput.map((server, serverIndex) => {
        ...
        if (!existing && !password) throw validationError(`Informe a senha SSH do novo servidor "${id}".`);
        if (password.length > 1000) throw validationError(`A senha SSH do servidor "${id}" é muito longa.`);
