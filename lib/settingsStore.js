...
const { normalizeSettings } = require('./validation');
...
class SettingsStore {
    ...
    _migrateRaw(rawInput) {
        ...
        const clientSecretEncrypted = this.security.isEncrypted(google.clientSecretEncrypted)
            ? google.clientSecretEncrypted
            : this.security.encrypt(
                typeof google.clientSecret === 'string'
                    ? google.clientSecret
                    : this.security.isEncrypted(google.clientSecret) ? this.security.decrypt(google.clientSecret) : ''
            );

        const refreshTokenEncrypted = this.security.isEncrypted(google.refreshTokenEncrypted)
            ? google.refreshTokenEncrypted
            : this.security.encrypt(
                typeof google.refreshToken === 'string'
                    ? google.refreshToken
                    : this.security.isEncrypted(google.refreshToken) ? this.security.decrypt(google.refreshToken) : ''
            );
        ...
        const migratedServers = servers.map(server => ({
            ...
            passwordEncrypted: this.security.isEncrypted(server.passwordEncrypted)
                ? server.passwordEncrypted
                : this.security.encrypt(
                    typeof server.password === 'string'
                        ? server.password
                        : this.security.isEncrypted(server.password) ? this.security.decrypt(server.password) : ''
                ),
